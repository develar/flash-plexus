 ...
 *
 * @see org.flyti.plexus.actions.builders.RemoteObjectInvoker
 * @see org.flyti.plexus.actions.builders.HTTPServiceInvoker
 * @see org.flyti.plexus.actions.builders.WebServiceInvoker
 */
public class ServiceInvoker extends AbstractServiceInvoker {
  ...
  override protected function complete(scope:IScope):void {
    innerHandlersDispatcher = IEventDispatcher(currentInstance);

    if (resultHandlers && resultHandlers.length > 0) {
      var resultHandlersInstance:ServiceHandlers = ServiceHandlers(createInnerHandlers(scope, ResultEvent.RESULT, resultHandlers, ServiceHandlers));
