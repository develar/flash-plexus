 ...
 * Если будет добавлен дескриптор с теми же координатами — role&roleHint, то: если у добавляемого есть requirements,то они заменят requirements существующего, если же их нет, то будут оставлены те, что уже есть.
 */
public class ComponentDescriptorRegistry {
  ...
  public function get(role:Class, roleHint:Enum):ComponentDescriptor {
    ...
    var descriptor:ComponentDescriptor = descriptorMap == null ? null : descriptorMap[role];
    return descriptor == null && roleHint != RoleHint.DEFAULT ? get(role, RoleHint.DEFAULT) : descriptor;
  }
}
}