...
public class RemoteObjectInvoker extends ServiceInvoker implements IAction {
  ...
  public function get instance():RemoteObject {
    return RemoteObject(currentInstance);
  }
  ...
  override protected function prepare(scope:IScope):void {
    super.prepare(scope);
    if (!currentInstance) {
      currentInstance = new RemoteObject();
    }
    var remoteObjectInstance:RemoteObject = RemoteObject(currentInstance);

    if (channelSet) {
      remoteObjectInstance.channelSet = channelSet;
    }
    if (concurrency) {
      remoteObjectInstance.concurrency = concurrency;
    }
    if (destination) {
      remoteObjectInstance.destination = destination;
    }
    if (endpoint) {
      remoteObjectInstance.endpoint = endpoint;
    }
    if (objectsBindableChanged) {
      remoteObjectInstance.makeObjectsBindable = makeObjectsBindable;
    }
    if (requestTimeoutChanged) {
      remoteObjectInstance.requestTimeout = requestTimeout;
    }
    if (showCursorChanged) {
      remoteObjectInstance.showBusyCursor = showBusyCursor;
    }
    if (source) {
      remoteObjectInstance.source = source;
    }
