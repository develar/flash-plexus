...
public class HTTPServiceInvoker extends ServiceInvoker implements IAction
{
	...
	public function set MXMLrequest(value:Object):void
	{
		request = value;
	}
	...
	{
		return HTTPService(currentInstance);
	}
	...
	override protected function prepare(scope:IScope):void
	{
		...
		var httpInstance:HTTPService = HTTPService(currentInstance);
		if (url)
		{
			httpInstance.url = (url is ISmartObject) ? ISmartObject(url).getValue(scope).toString() : url.toString();
		}
		if (request)
		{
			if (request is Request || request.constructor == Object || request is ISmartObject)
			{
				var smartRequest:Object = request is ISmartObject ? ISmartObject(request).getValue(scope) : request;
		...
		if (method != null)
		{
			httpInstance.method = String(method is ISmartObject ? ISmartObject(method).getValue(scope) : method);
		}
